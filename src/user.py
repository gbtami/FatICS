...
class BaseUser(object):
    ...
    def log_on(self, conn):
        self.vars.update(global_.varlist.get_transient_vars())
        self.aliases = {}
        self.gnotified = set()
        self.gnotifiers = set()
        self.noplay = set()
    ...
    def write_(self, s, args={}):
        ...
        self.session.conn.write(global_.langs[self.vars['lang']].gettext(s) %
            args)
        self.write_prompt()

    def nwrite_(self, s1, s2, n, args={}):
        self.session.conn.write(
            global_.langs[self.vars['lang']].ngettext(s1, s2, n) % args)
        self.write_prompt()

    def translate(self, s, args={}):
        return global_.langs[self.vars['lang']].gettext(s) % args
...
class GuestUser(BaseUser):
    def __init__(self, name):
        ...
        self.vars = global_.varlist.get_default_vars()
        assert('formula' in self.vars)
        self.censor = set()
        self.is_muted = False
